...
class trainer():
    ...
    def train(self, input, real_val):
        ...
        # print(input.shape)
        # 在最后一个维度左侧填充0,
        input = nn.functional.pad(input, (1, 0, 0, 0))  # torch.Size([64, 1, 207, 13])
        output = self.model(input)  # torch.Size([64, 12, 207, 1])
        output = output.transpose(1, 3)  # [64,1,207,12]

        # print(real_val.shape)
        # torch.Size([64, 170, 12]) -> torch.Size([64, 1, 170, 12])
        real = torch.unsqueeze(real_val, dim=1)
        predict = self.scaler.inverse_transform(output)  # torch.Size([64, 1, 170, 12])
    ...
    def eval(self, input, real_val):
        self.model.eval()
        input = nn.functional.pad(input, (1, 0, 0, 0))  # torch.Size([64, 2, 207, 13])
        output = self.model(input)  # torch.Size([64, 12, 207, 1])
        output = output.transpose(1, 3)  # [64,1,207,12]
